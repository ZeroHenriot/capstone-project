...
const PassedRace = ({ passedRace }) => {
  return (
    <>
      ...
      >
        {passedRace ? (
          <Link className="nav-link" to={`/schedule/race/${passedRace.id}`}>
            <Row className=" fontWide border-bottom pb-2 mx-2 justify-content-between">
              <span className="p-0" style={{ width: 'fit-content' }}>
                {`${format(
                  subDays(new Date(passedRace.date), 2),
                  'dd'
                )} - ${format(new Date(passedRace.date), 'dd')}`}
              </span>
              <div
              ...
              >
                <span
                  ...
                  style={{ width: 'fit-content' }}
                >{`${format(new Date(passedRace.date), 'MMM')}`}</span>
              </div>
            </Row>
            <Row className="border-bottom pb-2 mx-2">
              <span className="fontBold p-0">
                {passedRace.competition.location.country}
              </span>
              <span className="p-0 d-none d-xl-inline">
                {passedRace.competition.name}
              </span>
            </Row>
            <Row
              className=" pb-2 mx-2"
